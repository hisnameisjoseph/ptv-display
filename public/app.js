...
function refreshStationList(list) {
    list.innerHTML = "";
    const entries = Object.entries(STATIONS).filter(([, cfg]) => subsequenceMatch(stationQuery, cfg.label));
    if (entries.length === 0) {
...
function render() {
    ...
    for (const stop of lastPayload.stops) {
        ...
        if (isTrain) {
            ...
            if (showBody) {
                if (isGrid)
                    buildTrainGrid(section, stop, stationCfg);
                else
                    buildTrainStacked(section, stop, stationCfg);
            }
        }
        else {
            if (isGrid)
